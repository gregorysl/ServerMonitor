    ...
    public class MonitorController : Controller
    {
        ...
        public ActionResult GetCPU()
        {
            try
            {
                ...
                return Json(new { Date = DateTime.Now.ToString("G"), Value = Math.Round(CpuCounter.NextValue()) }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Response.StatusCode = 500;
        ...
        [HttpGet]
        public ActionResult GetFreeMemory()
        {
            try
            {
                double memAvailable, memPhysical;

                var p = new PerformanceCounter("Memory", "Available KBytes");
                memAvailable = (double)p.NextValue();
                memPhysical = GetPhysicalMemory();

                var totalMem = Math.Round((memPhysical - memAvailable) * 100 / memPhysical);

                return Json(new { Date = DateTime.Now.ToString("G"), Value = totalMem }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Response.StatusCode = 500;
        ...
        public ActionResult GetFreeSpace()
        {
            try
            {
                ...
                return Json(new { Date = DateTime.Now.ToString("G"), Value = Math.Round(p.NextValue()) }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Response.StatusCode = 500;
